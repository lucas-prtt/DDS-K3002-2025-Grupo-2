...
public interface DimensionColeccionRepository extends JpaRepository<DimensionColeccion, String> {
    ...
    @Query("SELECT d FROM DimensionColeccion d " +
            "WHERE LOWER(d.titulo) LIKE LOWER(CONCAT('%', :search, '%')) ")
    Page<DimensionColeccion> findSearch(@Param("search") String search, Pageable pageable);
}
